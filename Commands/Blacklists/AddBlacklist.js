  ...
  async execute(interaction) {
    ...
    if (!ip && !hwid) {
      return interaction.editReply({
        embeds: [
          new EmbedBuilder()
            .setDescription(
              `You need to either define hwid or ip paramater. Please try again defining one of these paramaters..`,
            )
            .setColor(Colors.Red)
            .setTimestamp(),
        ],
    ...
    if (ip && hwid) {
      return interaction.editReply({
        embeds: [
          new EmbedBuilder()
            .setDescription(`Please only define one paramater per command..`)
            .setColor(Colors.Red)
            .setTimestamp(),
        ],
        ephemeral: ephemeral,
